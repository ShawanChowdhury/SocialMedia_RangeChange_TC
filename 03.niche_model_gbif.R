...
for (i in yr) {
  ...
  write_csv(opt.aicc, file = paste0("output/sdm/gbif/gbif_model_sum_", i, ".csv"))
  ...
  writeRaster(r, paste0("output/sdm/gbif/gbif_map_", i, ".tif"), 
              NAflag=-9999, overwrite = TRUE)
  
  # Binary map [equal/over the threshold value]
  r_bin <- r >= threshold
  writeRaster(r_bin, paste0("output/sdm/gbif/gbif_bin_", i, ".tif"), 
              NAflag=-9999, overwrite = TRUE)
